...
for (t1,d) in Succ:
    for t2 in Trips:
        if TripDep[t2][d] and \
            TripTime[t2][0]>=TripTime[t1][1]+D[TripLoc[t1][1]][TripLoc[t2][0]] and \
            TripTime[t2][1]+TravTD[t2,d]-(TripTime[t1][0]-TravDT[d,t1])<=MaxDur:
            Succ[t1,d].add(t2)
            Pred[t2,d].add(t1)
...
Cover = {i:
    m.addConstr(quicksum(YS[i,d] for d in Depots if (i,d) in YS)+
                quicksum(X[j,i,d] for d in Depots if (i,d) in Pred 
                         for j in Pred[i,d])==1)
    for i in Trips}
    
ConserveFlow = {(i,d):
    m.addConstr(YS[i,d]+quicksum(X[j,i,d] for j in Pred[i,d])==
                YE[i,d]+quicksum(X[i,j,d] for j in Succ[i,d]))
    for (i,d) in Succ}
    
AverageLess8Hours = {d:
    m.addConstr(
        quicksum((TripTime[i][1]+TravTD[i,d])*YE[i,d] 
                  for i in Trips if (i,d) in YE)-
        quicksum((TripTime[i][0]-TravDT[d,i])*YS[i,d]
                  for i in Trips if (i,d) in YS)<=
