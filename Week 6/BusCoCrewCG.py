...
for (t1,d) in Succ:
    for t2 in Trips:
        if TripDep[t2][d] and \
            ...
            Succ[t1,d][t2] = TripTime[t2][0]>=TripTime[t1][1]+D[TripLoc[t1][1]][TripLoc[t2][0]]+MinBreak
            Pred[t2,d].add(t1)
...
def CheckLegal(d,p):
    if len(p)==1:
        return True
    if TripTime[p[-1]][1]+TravTD[p[-1],d]-(TripTime[p[0]][0]-TravDT[d,p[0]]) <= MaxDur:
        ...
        for t1,t2 in zip(p,p[1:]):
            if Succ[t1,d][t2] and \
                TripTime[t1][1]-TripTime[p[0]][0]<=MaxNoBreak and \
                TripTime[p[-1]][1]-TripTime[t2][0]<=MaxNoBreak:
                    return True
    return False
...
Cover = {t:
    m.addConstr(quicksum(Contains[t])==1)
    for t in Trips}
