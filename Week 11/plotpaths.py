...
def PlotPaths(pos, arrival_time, pred, solution, ignitions, target, plotsize):
    ...
    nodelabels = nx.draw_networkx_labels(graph, pos, {n: arrival_time[n] for n in nodes}, font_size=8)
    # nodelabels = nx.draw_networkx_labels(graph, pos, {n: n for n in nodes}, font_size=8)

    burns = [n for n in nodes if arrival_time[n] < target
             and n not in solution and n not in ignitions]
    ...
    drawignitions = nx.draw_networkx_nodes(graph, pos, nodelist = ignitions, node_color = 'thistle')
    drawburns = nx.draw_networkx_nodes(graph, pos, nodelist = burns, node_color = 'orangered')
    drawburnsres = nx.draw_networkx_nodes(graph, pos, nodelist = burnsres, node_color = 'firebrick')
    drawsafe = nx.draw_networkx_nodes(graph, pos, nodelist = safe, node_color = 'forestgreen')
    drawsaferes= nx.draw_networkx_nodes(graph, pos, nodelist = saferes, node_color = 'green')
