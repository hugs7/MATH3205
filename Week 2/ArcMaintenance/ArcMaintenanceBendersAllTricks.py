...
JobsTA = [
    [
        {(j, p) for j in JobsA[a] for p in JP[j] if p <= t and p + Jobs[j][3] > t}
        for a in A
    ]
    for t in T
]
...
FlowConservation = {
    n: BSP.addConstr(
        quicksum(X[a] for a in A if Arcs[a][0] == n)
        == quicksum(X[a] for a in A if Arcs[a][1] == n)
    )
    for n in N
}

UpperBound = {a: BSP.addConstr(X[a] <= Arcs[a][2]) for a in A}
...
EachJobScheduled = {j: BMP.addConstr(quicksum(Y[j, t] for t in JP[j]) == 1) for j in J}
...
while True:
    ...
    for t in T:
        for a in A:
            UpperBound[a].RHS = Arcs[a][2] * (
                1 - sum(Y[j, tt].x for (j, tt) in JobsTA[t][a])
            )
        BSP.optimize()
