...
\begin{itemize}
    ...
    \item $S2_{ep}$ the estimate of the penalty for assigning the event $e \in
        P2$ to an undesired room in period $p \in P$.
...
\begin{multline*}
    ...
    + \quad \text{NOT\_PREFERRED} \sum_{e \in PD} \sum_{p \in PP_e} (1 - y_{ep})
        \quad + \quad \sum_{e \in P2} \sum_{p \in P} S2_{ep} \quad + \quad
        \sum_{s \in P3} S3_s
\end{multline*}
...
\begin{itemize}
    ...
    \item If BSP has a non-zero objective value, there does not exist any way
        to allocate each of the events scheduled in the current period such that
        no event is in an undesired room. Indeed, the number of undesired room
        allocations has been minimized. Let $UR \subset E_p$ be the set of
        events allocated to an undesired room and, for each $e \in UR$ define
        $K_e := \set{e' \in E_p \cap P2 \given RM_{e'} \in RA_e}$. Observe that
        $e \in K_e$ and, whenever BMP schedules any set of events $\mathcal{E}$
        with $K_e \subseteq \mathcal{E}$ to the current period $p$, the penalty
        $S2_{ep}$ \emph{cannot} decrease. Therefore we add the following
        optimality cut:
        \begin{equation}
            S2_{e,p} \geq \text{UNDESIRED\_ROOM} \left(
                \sum_{e' \in K_e} y_{e',p} - |K_e| + 1
            \right) \qquad \forall e \in UR
        \end{equation}
        Notice that we cannot generalize this to every period because the rooms
        available is \emph{not} constant across periods.
\end{itemize}
...
The sub-problem will allocate events to rooms within a time period once they
have been scheduled by the master problem. In addition to the sets, data and
variables defined in the master problem, we fix $p \in P$ (and hence $E_p$) and
define the following.

\subsubsection*{Data}
Let $R_p$ be the set of rooms available in the current period.
...
\subsubsection*{Objective}
$$
    \min \qquad \sum_{e \in E_p \cap P2} \ \sum_{r \in UR_e \cap R_p} x_{e,r}
$$

\subsubsection*{Constraints}
Our only constraints are that each event is allocated exactly one room, and no
two events are allocated the same room:
\begin{gather*}
