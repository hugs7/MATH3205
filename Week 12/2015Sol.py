...
if PartB:
    ...
    MoveOut = {s1:
        m.addConstr(quicksum(X[s1,s2,t] for s2 in MoveTo[s1] for t in T)==1)
        for s1 in SOut}
    OnePerTurn = {t:
        m.addConstr(quicksum(X[s1,s2,t] for s1 in SOut for s2 in MoveTo[s1])==1)
        for t in T}
    InOnce = {s:
        m.addConstr(quicksum(X[k] for k in X if k[1]==s)<=1)
        for s in SIn}
    print("In Once done")
    # Only into a square in time t if moved out beforehand
    OutBeforeIn = {(t,s):
        m.addConstr(quicksum(X[s2,s,t] for s2 in SOut if (s2,s,t) in X)<=
                    quicksum(X[s,s2,td] for s2 in MoveTo[s] for td in T[:t]))
        for t in T for s in SOut}
        ...
        if where==GRB.Callback.MIPSOL:
            ...
            for s in SOut:
                ...
                if upto in LoopSet:
                    ...
                    print('Loop:', LoopSet)
                    model.cbLazy(quicksum(X[s1,s2] 
                            for s1 in LoopSet 
                            for s2 in LoopSet if (s1,s2) in X)<=
                                 len(LoopSet)-1)
    ...
    MoveOut = {s1:
        m.addConstr(quicksum(X[s1,s2] for s2 in MoveTo[s1])==1)
        for s1 in SOut}
    InOnce = {s:
        m.addConstr(quicksum(X[k] for k in X if k[1]==s)<=1)
        for s in SIn}
