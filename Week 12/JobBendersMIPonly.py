...
def Callback(model, where):
    ...
    if where==gp.GRB.Callback.MIPSOL:
        ...
        for m in machines:
            ...
            minProc = min(processing_time[m][j] for j in J)
            
            # Start time variables
            S = {(j, t): BSP.addVar(
                vtype=gp.GRB.BINARY) for j in J 
                for t in range(deadline - processing_time[m][j]+1)
                if t==0 or t>=minProc}
            
            A = {t: BSP.addVar() for t in range(minProc-1,deadline-minProc+1)}
            BSP.addConstr(A[minProc-1]==capacity[m]-gp.quicksum(consume[j]*S[j,0] for j in J))
            EndJ = {t:[] for t in range(minProc,deadline-minProc+1)}
            StartJ = {t:[] for t in range(minProc,deadline-minProc+1)}
            for (j,t) in S:
                if t>=minProc:
                    StartJ[t].append((j,t))
                if t+processing_time[m][j]<=deadline-minProc:
                    EndJ[t+processing_time[m][j]].append((j,t))
            for t in EndJ:
                BSP.addConstr(A[t]==A[t-1]+
                        gp.quicksum(consume[j]*S[j,tt] for (j,tt) in EndJ[t])-
                        gp.quicksum(consume[j]*S[j,tt] for (j,tt) in StartJ[t]))
            
            # Constraints by job
            for j in J:
                BSP.addConstr(gp.quicksum(S[j,t] 
                        for t in range(deadline) if (j,t) in S)==1)
            ...
            for j in J:
                ...
                BSP.addConstr(makespan_m >= gp.quicksum(
                    (t + processing_time[m][j]) * S[j, t]
                    for t in range(deadline) if (j,t) in S))
